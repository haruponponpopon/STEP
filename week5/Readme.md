...
```
### two_opt関数  
まずstartの都市と最後の都市を見て交差していたら交差を解きます。その後二重ループで全ての隣接する都市の組み合わせについて交差していたら解くことを繰り返します。表の四番目にj=0との記述がありますが、交差を解いたら新しい交差ができてしまい貪欲法のみを用いた時より出力される経路が長くなってしまうバグが生じたので、交差したら再び最初から交差がないかどうか見ていきます。しかしこの方法でも都市の数が大きかった時に無限ループが生じてしまったのでループの上限を設けました。そうしたら表にある通り、都市の数によってはうまくいかないことがありました。(原因は考えましたがこれ以上は難しいので提出させていただきました。)
### write_file関数  
